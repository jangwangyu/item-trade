...
public class CommentService {
   ...
   * @return 작성된 댓글 정보
   */
  public CommentDto addComment(CommentCreateRequest request, Long postId, CustomOAuth2User member) {
    ItemPost itemPost = itemPostRepository.findById(postId).orElseThrow(() ->
        new IllegalArgumentException("게시물이 존재하지 않습니다."));
    Member writer = memberRepository.findById(member.getMember().getId()).orElseThrow(() ->
   ...
   * @param member    삭제 요청자
   */
  public Long deleteComment(Long commentId, CustomOAuth2User member) {
    Comment comment = commentRepository.findById(commentId).orElseThrow(() ->
        new IllegalArgumentException("댓글이 존재하지 않습니다."));
    if(!comment.getWriter().getId().equals(member.getMember().getId())) {
